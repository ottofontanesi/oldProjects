...
├── src-tauri/src/              # Rust backend (~90 source files)
...
│   │   ├── split/             # Split inference protocol (distributed layers)
│   │   └── local/             # Local llama.cpp backend (feature-gated)
│   ├── integration/           # RL-optimizer integration (ONNX, encoder, decoder)
...
├── .github/workflows/         # CI: typecheck.yml (Rust + TypeScript parallel)
...
- All 14 integration specs + 3 research/architecture specs implemented
- Total Rust modules: 15 directories, 110+ source files
- Property-based tests: 80+ across all modules
- Feature gates: `tract-onnx`, `backend-llamacpp`, `backend-onnx`, `backend-tenstorrent`, `backend-ascend`
- CI: GitHub Actions workflow at `.github/workflows/typecheck.yml`
