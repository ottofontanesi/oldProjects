...
// --- Scoring Engine Advisory Integration (post-hoc) ---
// The scoring engine runs on a background thread and produces recommendations
// that are evaluated AFTER the heuristic router makes its decision.
// This integration never blocks the heuristic decision path.

import type { ScoringRecommendation } from "./scoring-engine";
import type { AdvisoryDecision, AdvisoryIntegrationConfig } from "./scoring-advisory";
import { evaluateAdvisory, shouldAttemptScoring } from "./scoring-advisory";

/**
 * Post-hoc advisory evaluation for a routing decision.
 * Runs after the heuristic router has already made its decision.
 * Returns the advisory decision without blocking the heuristic path.
 *
 * The scoring engine recommendation is evaluated with a 50ms timeout.
 * If scoring is unavailable or times out, the heuristic decision stands unchanged.
 */
export const evaluateRoutingAdvisory = (
  heuristicDecision: ProviderRoutingDecision,
  scoringRecommendation: ScoringRecommendation | null,
  config: AdvisoryIntegrationConfig,
): AdvisoryDecision => {
  // Check circuit breaker before attempting evaluation
  if (!shouldAttemptScoring(config.circuitBreakerState, new Date().toISOString())) {
    return {
      accepted: false,
  ...
  return evaluateAdvisory(scoringRecommendation, heuristicDecision, config);
};
