...
def updater(
    ...
    if exit_status == -1:
        logger.debug("Skip {}-{} : downloader exit_status = {}".format(index_id, stock_country_id, exit_status))
        return exit_status

    ts_new = pd.read_parquet(_SERIES_TMP_PATH_ / f"{index_id}_{stock_country_id}_ts.parquet", engine="fastparquet")

    # Handle case where old file doesn't exist yet
    old_file_path = _SERIES_PATH_ / f"{index_id}_{stock_country_id}_ts.parquet"
    if old_file_path.exists():
        ts_old = pd.read_parquet(old_file_path, engine="fastparquet")
    else:
        ts_old = pd.DataFrame()
    ...
    # Fixed: use independent if/else blocks instead of broken elif chain
    if ts_old.shape[0] > 0:
...
def get_index_name_old(country_id: int, index_id: int, payloads: dict):
    ...
    warnings.warn("get_index_name_old is deprecated, use get_index_name instead", DeprecationWarning, stacklevel=2)
...
def downloader(
    ...
    anagrafica_dir = raw_file_path / "anagrafica"
    series_dir = raw_file_path / "series"
    anagrafica_dir.mkdir(parents=True, exist_ok=True)
    series_dir.mkdir(parents=True, exist_ok=True)

    df_anagrafica.to_parquet(anagrafica_dir / f"{index_id}_{stock_country_id}_anagrafica.parquet")
    ts_df.to_parquet(series_dir / f"{index_id}_{stock_country_id}_ts.parquet", engine="fastparquet")
    return 0
    ...
    for i, prod in enumerate(prod_list):
        try:
            ...
            if time_span == "1D":
                ...
                raise ValueError(f"Unsupported time_span: {time_span}")
...
def set_threshold_2(v_r: list, lower_boundary=-0.025, upper_boundary=0.025):
    v_t = []
    for r in v_r:
        if r < lower_boundary:
            v_t.append("SELL")
...
def set_threshold_3(v_r: list, lower_boundary=-0.025, upper_boundary=0.025):
    v_t = []
    for r in v_r:
        if r < lower_boundary:
            v_t.append(-1)
...
def time_series_builder(df):
    """Build time series DataFrame using outer join to preserve all dates."""
    ts_df = pd.DataFrame(index=df.iloc[0].TIME_SERIES.index, columns=["--"], data=-100)
    for _, row in df.iterrows():
        ts_df = ts_df.merge(row["TIME_SERIES"], how="outer", left_index=True, right_index=True)
...
def min_max_scaler(ts, ts_min: float, ts_max: float):
    if ts_max == ts_min:
        logger.debug("min_max_scaler: ts_min == ts_max, returning zeros")
        return np.zeros_like(ts)
    return (ts - ts_min) / (ts_max - ts_min)


def min_max_scaler_inverse(ts, ts_min: float, ts_max: float):
...
def from_dict_values_to_hash(dictionary: dict):
    """Hash dictionary values + keys for uniqueness."""
    try:
        # Include keys in hash to avoid collisions between dicts with same values but different keys
        list_to_hash = [str(k) + "=" + str(v) for k, v in sorted(dictionary.items())]
        str_to_hash = "|".join(list_to_hash)
        return hashlib.md5(str_to_hash.encode()).hexdigest()
    except Exception as e:
        logger.debug(f"from_dict_values_to_hash error: {e}, dictionary: {dictionary}")
        return None


def build_dict_for_hashing(orig_param, new_param):
    """Merge two dicts for hashing WITHOUT mutating the original."""
    merged = dict(orig_param)
    merged.update(new_param)
    return merged
...
def train_models_on_df(
    ...
    chunks_num = model_param["chunks_num"]
    total_cols = len(ts.columns)
    # Fixed: use ceiling division to avoid dropping remainder stocks
    chunks_len = math.ceil(total_cols / chunks_num)
    ...
    for chunk_num in range(chunks_num):
        logger.debug("Start training chunk...{}".format(chunk_num + 1))
        start_idx = chunk_num * chunks_len
        end_idx = min(start_idx + chunks_len, total_cols)
        columns = ts.columns[start_idx:end_idx]
        if len(columns) == 0:
            break
        logger.debug(f"Working on : {list(columns)}")
    ...
    rnn_models[["TRAIN_LOSS", "TEST_LOSS", "VOL_IS", "VOL_OOS"]] = rnn_models.apply(
        lambda x: rnn_get_stats_2(model=x.MODEL, dataset=ts[x.name], look_back=model_param["look_back"]),
    ...
    logger.debug("Writing session to parquet...")
...
def train_on_grid(
    ...
    for iteration, model_p in enumerate(param_permutations):
        ...
        logger.debug("Done {} -> {}%".format(iteration, (iteration) / len(param_permutations) * 100))
...
def compute_log_ret(ts: pd.DataFrame, price_threshold: float = 100.0):
    """
    :param ts: dataframe con vettore prezzi in colonna ordinati per data decrescente
    :param price_threshold: soglia di prezzo sotto la quale considero la serie storica
    """
    max_price = ts.apply(max)
    ts_cutted = ts[max_price[max_price < price_threshold].index]
    if ts_cutted.empty:
        logger.debug("compute_log_ret: no stocks below price_threshold={}".format(price_threshold))
        return ts_cutted
...
def build_predicted_df(models_df: pd.DataFrame, ts: pd.DataFrame, look_back: int):
    """
    Build predicted DataFrame from trained models and time series.
    """
    ...
    for i, obs_date in enumerate(ts.index[:len(date_col)]):
        if obs_date >= last_date:
            logger.debug(f"{obs_date} >= {last_date}")
            break
        try:
            date_window = ts.index[i:(i + look_back)]
            cols = ss.columns[ss.columns.get_level_values(level=0) == f"SAMPLE_{i}"]
            ss[cols] = ts.loc[date_window].T.astype(np.float64)
        except Exception as e:
            logger.debug(f"build_predicted_df error at sample {i}: {e}")
